...
const double gstar_HSDM = 2*(Nc*Nc-1)+2*Nf*Nf;
const double gstar_SM = 106.75;
const double gstar = gstar_SM + gstar_HSDM;
const double Om_HSDM = gstar_HSDM / (gstar_HSDM+gstar_SM);
const double MP_GeV = 1.220890e19; //GeV https://physics.nist.gov/cgi-bin/cuu/Value?plkmc2gev
...
struct F_Tc {
  ...
  F_Tc(const std::vector<double> fp_)
    : fp(fp_)
  {}
...
struct F_DeltaV{
  ...
  F_DeltaV(const std::vector<double> fp_)
    : fp(fp_)
  {}
...
struct F_S3hat{
  ...
  F_S3hat(const std::vector<double> fp_)
    : fp(fp_)
  {}
...
double getvJ(const double alpha){
  assert(alpha>0.0);
...
// #endif
  for(int iT=0; iT<NTs; iT++) {
    ...
    for(int iDT1=0; iDT1<NTs-iT; iDT1++) {
      ...
      std::cout << "debug4. " << Hinvs[iT1prime] << " " << GamPrimes[iT1prime] << " " << T_T1_Ints[iT][iDT1] << " " << std::pow(T_T1_Ints[iT][iDT1], 3) << std::endl;
      std::cout << "debug4. " << Hinvs[iT1prime+1] << " " << GamPrimes[iT1prime+1] << " " << T_T1_Ints[iT][iDT1+1] << " " << std::pow(T_T1_Ints[iT][iDT1+1], 3) << std::endl;

      std::cout << "debug3. " << integral << std::endl;
      if(integral>1.0e40) {
...
void get_freq_amplitude( double& fpeak, double& hsq_Omega_peak,
  ...
  fpeak = 1.9e-5 * std::pow(gstar/100.,1.0/6.0) * (Tstar*sqrt_t0_inv_GeV/100.) * (beta_tilde / vJ);
  hsq_Omega_peak = 2.65e-6 * (vJ/beta_tilde) * std::pow(kappa_sw*alpha/(1.0+alpha),2) * std::pow(100./gstar,1.0/3.0) * std::pow(Om_HSDM,2);
}
